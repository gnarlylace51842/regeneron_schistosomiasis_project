#!/usr/bin/env python3
"""Train a minimal always-on BF+DF baseline from metadata/pairs.csv."""
...
from schisto_mobile_ai.data.paired_classification import PairedContrastDataset, load_dual_contrast_data
...
def build_parser() -> argparse.ArgumentParser:
    """Build the command-line parser for always-on dual-contrast training."""
    ...
    parser.add_argument(
        ...
        help="Root directory used with pairs.csv relative paths.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def _move_batch_to_device(batch: dict[str, Any], device: str) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict[str, Any]]:
    brightfield_image = batch["brightfield_image"].to(device)
    darkfield_image = batch["darkfield_image"].to(device)
    target = batch["target"].to(device)
    metadata = {
        key: value
        for key, value in batch.items()
        if key not in {"brightfield_image", "darkfield_image", "target"}
    }
    return brightfield_image, darkfield_image, target, metadata
...
def train_one_epoch(
    ...
    *,
    ...
    for batch in loader:
        brightfield_image, darkfield_image, targets, _ = _move_batch_to_device(batch, device)
        optimizer.zero_grad(set_to_none=True)
        logits = model(brightfield_image, darkfield_image).squeeze(1)
...
def evaluate(
    ...
    *,
    ...
    """Evaluate on the validation loader and return metrics plus per-pair predictions."""
    model.eval()
    total_loss = 0.0
    ...
    with torch.no_grad():
        for batch in loader:
            brightfield_image, darkfield_image, targets, metadata = _move_batch_to_device(batch, device)
            logits = model(brightfield_image, darkfield_image).squeeze(1)
            loss = criterion(logits, targets)
            ...
            for index in range(len(batch_targets)):
                rows.append(
                    {
                        "image_id": metadata["pair_id"][index],
                        "pair_id": metadata["pair_id"][index],
                        "pair_key": metadata["pair_key"][index],
                        ...
                        "brightfield_relative_path": metadata["brightfield_relative_path"][index],
                        "darkfield_relative_path": metadata["darkfield_relative_path"][index],
                    }
                )
    ...
    *,
    ...
    snapshot["training"] = {
        **snapshot.get("training", {}),
        "task_type": "binary_pair_classification",
        "backbone": model_name,
        "epochs": epochs,
...
def main() -> int:
    """Train the always-on dual-contrast baseline."""
    parser = build_parser()
    ...
    data_bundle = load_dual_contrast_data(
    ...
    train_dataset = PairedContrastDataset(data_bundle.train_frame, image_size=img_size, train=True)
    val_dataset = PairedContrastDataset(data_bundle.val_frame, image_size=img_size, train=False)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if np.isfinite(val_metrics["patient_auc_mean"]):
            selection_score = val_metrics["patient_auc_mean"]
        elif np.isfinite(val_metrics["auc"]):
        ...
        logger.info(
            "Epoch %s/%s | train_loss=%.4f | val_loss=%.4f | val_pair_auc=%s | val_patient_auc_mean=%s",
            epoch,
            epochs,
            train_metrics["loss"],
            val_metrics["loss"],
            f"{val_metrics['auc']:.4f}" if np.isfinite(val_metrics["auc"]) else "nan",
            f"{val_metrics['patient_auc_mean']:.4f}" if np.isfinite(val_metrics["patient_auc_mean"]) else "nan",
        )
    ...
    print("Dual-Contrast Training Summary")
    print(f"  device: {device}")
    print(f"  train_pairs: {data_bundle.metadata['n_train_pairs']}")
    print(f"  val_pairs: {data_bundle.metadata['n_val_pairs']}")
